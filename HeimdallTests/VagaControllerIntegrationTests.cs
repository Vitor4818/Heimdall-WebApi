...
namespace HeimdallTests
{
    public class VagaControllerIntegrationTests : IDisposable
    {
        ...
        private async Task SetupZonaEVagaLivreAsync(int zonaId = 1, int vagaId = 1, string codigoVaga = "V1")
        {
            ...
            (await _client.PostAsJsonAsync($"/api/v1/vaga", vaga)).EnsureSuccessStatusCode();
        }
        ...
        private async Task OcuparVagaComMotoAsync(int vagaId, int motoId)
        {
            ...
            (await _client.PostAsJsonAsync($"/api/v1/motos", moto)).EnsureSuccessStatusCode();
        }
        ...
        {
            // Act
            var response = await _client.GetAsync("/api/v1/vaga");
        ...
        public async Task Post_Vaga_DeveCadastrarVaga_EIgnorarOcupada()
        {
            ...
            // Act
            var response = await _client.PostAsJsonAsync("/api/v1/vaga", novaVaga);
        ...
        public async Task Put_Vaga_DeveAtualizarVaga_EIgnorarOcupada()
        {
            ...
            // Act
            var putResponse = await _client.PutAsJsonAsync("/api/v1/vaga/1", vagaAtualizada);

            // Assert
            putResponse.EnsureSuccessStatusCode(); 
            Assert.Equal(HttpStatusCode.NoContent, putResponse.StatusCode);
            
            var getResponse = await _client.GetAsync("/api/v1/vaga/1");
        ...
        public async Task Delete_Vaga_DeveFalhar_SeVagaEstiverOcupada()
        {
            ...
            // Act
            var deleteResponse = await _client.DeleteAsync("/api/v1/vaga/1");
        ...
        public async Task LiberarVaga_DeveRetornarOk_QuandoVagaOcupada()
        {
            ...
            // Act
            var patchResponse = await _client.PatchAsync("/api/v1/vaga/1/liberar", null);
        ...
        {
            // Act
            var patchResponse = await _client.PatchAsync("/api/v1/vaga/99/liberar", null);
